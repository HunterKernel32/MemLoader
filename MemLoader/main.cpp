...
int main()
{
    ...
    pfnNtSetInformationProcess NtSetInformationProcess = (pfnNtSetInformationProcess)
    ...
    ULONG ExecuteFlags = 0; ULONG retLen;
    NtQueryInformationProcess(GetCurrentProcess(), ProcessExecuteFlags, &ExecuteFlags, 4, &retLen);

    printf("%d\n", ExecuteFlags); ExecuteFlags = 2;
    NTSTATUS status = NtSetInformationProcess(GetCurrentProcess(), ProcessExecuteFlags, &ExecuteFlags, 4);
    ...
    PVOID res = AddVectoredExceptionHandler(TRUE, [](PEXCEPTION_POINTERS ExceptionInfo)->LONG {
        auto expHandle = ((PEXCEPTION_REGISTRATION_RECORD)(ExceptionInfo->ContextRecord->Ebp - 0x10));
        auto curExpRegistration = (PEXCEPTION_REGISTRATION_RECORD)__readfsdword(0);
        printf("expHandle = %p\n", expHandle);
        printf("curExpRegistration = %p\n", curExpRegistration);
        if (expHandle == curExpRegistration)
        {
            if (expHandle->Handler(ExceptionInfo->ExceptionRecord, curExpRegistration, ExceptionInfo->ContextRecord, NULL) == ExceptionContinueExecution)
            {
